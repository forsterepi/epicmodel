...
The steplist is the structure that contains all specified steps. It is the only input for function `create_scc()`, which creates the SCC model. See `?epicmodel_steplist` for a detailed description of the structure of steplists from a R perspective.
...
In R, steplists are defined as S3 class and contain 8 data.frames. Here's a short overview, but also see `?epicmodel_steplist` for details.
...
We made a little tutorial that shows how to use the `shiny` app, see `vignette("steplist_creator_tutorial")`. It contains screenshots and an example for you to click along. Please note that the tutorial is not shipped with the package and can only be accessed on the homepage.
