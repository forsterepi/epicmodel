...
-   For a detailed description of the structure of SCC objects from a R perspective, see `?epicmodel_scc`.
...
The final step of the aforementioned algorithm is called "Add unknown causes". As described above, a SCC model must, in theory, contain all causes, component causes as well as sufficient causes, of the outcome of interest. In practice, knowing all causes is of course unrealistic. `create_scc()` therefore adds unknown causes as placeholders. Two types of unknown causes are added (see also `?epicmodel_scc`):
